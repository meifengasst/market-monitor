...
# 1. 鑰匙與配置 (請確保 GitHub Secrets 已設定)
...
# 2. AI 讀報：直球 REST API 版
def get_ai_summary(stock_name, news_list, bias):
    if not GEMINI_API_KEY or not news_list: return "目前市場靜悄悄，無最新新聞。"
    headlines = [n.get('title', '') for n in news_list[:5]]
    # 讓 AI 根據乖離率給出操作建議
    prompt = f"你是台股資深股神阿土伯。標的【{stock_name}】，乖離率{bias}%。請根據數據與以下新聞，用繁體中文寫25字內的一句話『操作建議』（如：分批獲利、等待拉回或撿便宜）：\n" + "\n".join(headlines)
    
    url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key={GEMINI_API_KEY}"
    try:
        res = requests.post(url, json={"contents": [{"parts": [{"text": prompt}]}]}, timeout=10)
        return res.json()['candidates'][0]['content']['parts'][0]['text'].strip()
    except: return "🤖 阿土伯正在盯盤，暫無評論。"
...
def analyze():
    ...
    for symbol, name in STOCKS.items():
        try:
            ...
            ma5, ma60 = close.rolling(5).mean().iloc[-1], close.rolling(60).mean().iloc[-1]
            ...
            ai_text = get_ai_summary(name, ticker.news, bias)
